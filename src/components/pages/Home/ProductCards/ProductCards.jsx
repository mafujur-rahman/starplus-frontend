...
const ProductCards = () => {
  ...
  const filteredProducts = data.products.filter((product) => {
    ...
    const brandMatch = selectedBrand ? product.category === selectedBrand : true;
  ...
  const handlePageChange = (newPage) => {
    if (newPage > 0 && newPage <= Math.ceil(data.total / 9)) {
      setPage(newPage);
    }
  };
  ...
  const pageButtons = [];
  for (let i = 1; i <= Math.ceil(data.total / 9); i++) {
    pageButtons.push(
      <button
        key={i}
  ...
  return (
    <div className="py-8 px-4 md:px-8 lg:px-16 bg-gradient-to-r from-[#95D2B3] via-[#D8EFD3] to-[#55AD9B] min-h-screen">
      ...
      <div className="w-full flex justify-center mt-10 mb-4">
        <button onClick={() => handlePageChange(page - 1)} disabled={page === 1} className="btn bg-[#55AD9B] text-white mr-2">Previous</button>
        {pageButtons}
        <button onClick={() => handlePageChange(page + 1)} disabled={page === Math.ceil(data.total / 9)} className="btn bg-[#55AD9B] text-white ml-2">Next</button>
      </div>
    </div>
  );
};
